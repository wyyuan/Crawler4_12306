...
{
    public partial class TimeDiagram : Form
    {
        //控制数据采集的线程  
        ...
        //初始化窗体
        public TimeDiagram(List<QueryItem> _queryList)
        {
            InitializeComponent();
        ...
        private int ExecuteQuery(int i, List<QueryItem> tempQueryList)
        {
            ...
            foreach (QueryItem q in tempQueryList)
            {
                ...
                if (q.date == null)
                {
                    date = DateTime.Now.Year + "-" + string.Format("{0:D2}", DateTime.Now.Month) + "-" + string.Format("{0:D2}", DateTime.Now.Day + 1);
                }
                else
                {
                    date = q.date.Year + "-" + string.Format("{0:D2}", q.date.Month) + "-" + string.Format("{0:D2}", q.date.Day); 
                }
                //查询OD之间所有车次
                ...
                foreach (LeftTicketItem lti in list)
                {
                    ...
                    {
                        PrintTextBox(string.Format("查询车次：{0}失败", lti.station_train_code));
                        continue;
                    }
                    if (tdiList == null)
                    {
                    ...
                    PrintTextBox(string.Format("查询车次：{0}成功", lti.station_train_code)); 
                ...
                dt.TableName = "diag_" + q.date.Month+"_"+ q.date.Day + "_" + Util.StaDict[q.fromStation].station_name + "_" + Util.StaDict[q.toStation].station_name + "_"  + i++.ToString();
                ds.Tables.Add(dt);
                DataSet2CSV.Export2CSV(ds, dt.TableName, true, diagrampath + dt.TableName + ".csv");
                PrintTextBox(string.Format("查询OD{0}，{1}结束", Util.StaDict[q.fromStation].station_name, Util.StaDict[q.toStation].station_name));
            }
            return i;
        }
